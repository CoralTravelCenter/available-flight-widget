...
rebuildDestinationsListWithData = (list) ->
    ...
    selectDestinationItem $container.find('.item').toArray().find (item) -> $(item).text() == prefer

rebuildAirportsListWithData = (list) ->
    $container = $('.data-column.destination-airport-closest-date .scrollable')
    $container.empty()
    $container.append list.map (item_data) -> "<div class='item dbl'><span>#{ item_data.ToAreaName }</span><span>#{ moment(Number(item_data.FlightDate.replace(/\D/g,''))).format('DD.MM.YYYY') }</span></div>"
    $container.get(0).perfectscrollbar?.update()

updateSelectionInfo = () ->
    setTimeout ->
        $container = $ctx.find('.foot .selection-info').empty()
        items_html = $ctx.find('.data-column .item.selected').toArray().map (item) ->
            $item = $(item)
